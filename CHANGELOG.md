...
- **Removed dead Anthropic API handling from the service worker.** `sw.js` had a fetch-handler guard that let `api.anthropic.com` requests bypass the cache for "the app-level AI queue" — that queue no longer exists, so the guard is gone. Bumped `CACHE_NAME` to `gainpath-v2` so the updated shell replaces the cached one.
